  ...
  void initState() {
    passwordFocusNode.addListener(() {
      if(passwordFocusNode.hasFocus){
        setState(() {
          passwordBorder = Colors.purple[800];
        });
      }else{
        setState(() {
    ...
    return loading? const Loading(firstScreen: true): Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: <Widget>[
            Padding(child: const Text("User Account", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 22.0),),padding: EdgeInsets.fromLTRB(20.0, 20.0, 0.0, 0.0),),
          Padding(
              ...
              child: Form(
                  ...
                  child: Column(
                    children: [
                      TextFormField(
                        autofocus: true,
                        decoration: editTextDecoration(hint: "Enter Name or Email"),
                        validator: (data) {
                          if(data != null){
                            if(data.length > 2){
                              return null;
                        ...
                        children: [
                          ...
                          Expanded(
                            ...
                            child: IconButton(
                                    ...
                                    }, icon: Icon(visibilityIcon, color: Colors.purple[800]),
                            )
                          )
                        ],
                      ))
                          ...
                          onPressed: () async{
                            ...
                            if(_formKey.currentState != null){
                              if(_formKey.currentState!.validate()){
                                ...
                                SaloonUser? result = await _auth.signInWithEmailAndPassword(username, password);
                                if(result == null){
                                  setState(() {
                                    loading =false;
                                    couldntLogIn = true;
                      ...
                      TextButton(onPressed: () {
                        widget.toggleView();
                      }, child: const Text("Don't have an account yet?", style: TextStyle(fontSize: 16.0, decoration: TextDecoration.underline, color: Colors.blue),))
                    ],
                  ),
                ),
              )
